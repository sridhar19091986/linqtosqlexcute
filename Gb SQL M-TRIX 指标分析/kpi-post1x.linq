...
set @name='WSP_Post_Repeat'
select @num=sum(Post_Repeat),@delay=avg(Post_Repeat_delayFirst) from Gb_Post1x
insert into #temp(mName,mNumber,mDelay,mRate) 
values(@name, @num,@delay,(@num+0.0)/@total)

set @name='RADIO_Status'
select @num=sum(RADIO_Status),@delay=avg(RADIO_Status_delayFirst) from Gb_Post1x
insert into #temp(mName,mNumber,mDelay,mRate) 
values(@name, @num,@delay,(@num+0.0)/@total)

set @name='LLC_Discarded'
select @num=sum(LLC_Discarded),@delay=avg(LLC_Discarded_delayFirst) from Gb_Post1x
insert into #temp(mName,mNumber,mDelay,mRate) 
values(@name, @num,@delay,(@num+0.0)/@total)

set @name='PDP_Deactivate'
select @num=sum(PDP_Deactivate),@delay=avg(PDP_Deactivate_delayFirst) from Gb_Post1x
insert into #temp(mName,mNumber,mDelay,mRate) 
values(@name, @num,@delay,(@num+0.0)/@total)

set @name='Detach_Request'
select @num=sum(Detach_Request),@delay=avg(Detach_Request_delayFirst) from Gb_Post1x
insert into #temp(mName,mNumber,mDelay,mRate) 
values(@name, @num,@delay,(@num+0.0)/@total)

set @name='Abort_Before'
select @num=sum(Abort_Before),@delay=avg(Abort_Before_delayFirst) from Gb_Post1x
insert into #temp(mName,mNumber,mDelay,mRate) 
values(@name, @num,@delay,(@num+0.0)/@total)

set @name='Disconnect_Before'
select @num=sum(Disconnect_Before),@delay=avg(Disconnect_Before_delayFirst) from Gb_Post1x
insert into #temp(mName,mNumber,mDelay,mRate) 
values(@name, @num,@delay,(@num+0.0)/@total)

set @name='Reply'
select @num=sum(Reply),@delay=avg(Reply_delayFirst) from Gb_Post1x
insert into #temp(mName,mNumber,mDelay,mRate) 
values(@name, @num,@delay,(@num+0.0)/@total)

set @name='Abort_After'
select @num=sum(Abort_After),@delay=avg(Abort_After_delayFirst) from Gb_Post1x
insert into #temp(mName,mNumber,mDelay,mRate) 
values(@name, @num,@delay,(@num+0.0)/@total)


set @name='Disconnect_After'
select @num=sum(Disconnect_After),@delay=avg(Disconnect_After_delayFirst) from Gb_Post1x
insert into #temp(mName,mNumber,mDelay,mRate) 
values(@name, @num,@delay,(@num+0.0)/@total)
