...
var totalAttatchRequest=Gb_Attachs.Where(e=>e.LAC==e.Gmm_lac).Where(e=>e.Request_id2=="IMSI");

var imsi=from p in totalAttatchRequest
         group p by p.IMSI into imsiReq
		 select new 
		 {
		 mKey=imsiReq.Key ,
		 mCount=imsiReq.Count()
		 };

imsi.OrderByDescending(e=>e.mCount).Dump();