...
void Main()
		{
			...
			Get(roo, path,myList  );
							...
							node = null;
							ShoppingList convList = new ShoppingList();
		                    foreach (var cdr in myList.Items)
		                     {
						    	for (int x = 0; x <= nodes.Count - 1; x++)
						    	{
								  if (nodes.Item(x).Attributes.Item(0).Value == cdr.name )
								{
	       {
		   node = nodes.Item(x);
		     string id=cdr.mess.Replace (" message ","") ;
			 string field=node.Attributes.Item(3).Value.ToString();
			 string text=cdr.price ;
			 string format=node.Attributes.Item(2).Value.ToString();
	         convList.AddItem (new Item (id ,field ,text,format));
	        }
...
//		  	myList.Dump ();
		 ...
		 XmlSerializer s = new XmlSerializer( typeof( ShoppingList ) );
			...
			s.Serialize(w,   convList);
			w.Close();

			// Deserialization
			ShoppingList newList;
			TextReader r = new StreamReader(@"c:\list.xml");
			  convList = (ShoppingList)s.Deserialize(r);
			r.Close();
		...
		public class Item
		{
			...
			get { return hexdecoder(this.price,this.format  ); }}
