...
var ps= from p in Gb_Connect1xes
	...
	//	mTime=psbyfilenum.Where (e=>e.FileNum ==psbyfilenum.Key).Select(e=>e.PacketTime.Value.AddHours(8)).Min (),
		mTime=psbyfilenum.Where (e=>e.FileNum ==psbyfilenum.Key).Select(e=>e.PacketTime.Value.AddHours(-8)).Min (),
	mName="Connect1x_Acknowledge",
	mMessage=psbyfilenum.Where (e=>e.FileNum ==psbyfilenum.Key).Where (e=>e.Acknowledge!=null).Select(e=>e.Acknowledge).Sum (),
	mSuccess= (psbyfilenum.Where (e=>e.FileNum ==psbyfilenum.Key).Where (e=>e.Acknowledge!=null).Select(e=>e.Acknowledge).Sum ()+0.0)/psbyfilenum.Where (e=>e.FileNum ==psbyfilenum.Key).Select(e=>e.Connect1x).Sum (),
	mDelay= psbyfilenum.Where (e=>e.FileNum ==psbyfilenum.Key).Where (e=>e.Acknowledge!=null).Select(e =>e.Acknowledge_delayFirst).Average ()
	};
	ps.Dump ();