...
var ps= from p in Gb_DNS
	group p by p.DumpFor into psbyfilenum
...
//	mTime=psbyfilenum.Where (e=>e.FileNum ==psbyfilenum.Key).Select(e=>e.PacketTime.Value.AddHours(8)).Min (),
		mTime=psbyfilenum.Where (e=>e.DumpFor ==psbyfilenum.Key).Select(e=>e.PacketTime.Value.AddHours(-8)).Min (),
	mName="DNS_Query_Response",
	mMessage= psbyfilenum.Where (e=>e.DumpFor ==psbyfilenum.Key).Where (e=>e.Response!=null).Select(e=>e.Response).Sum (),
	mSuccess= (psbyfilenum.Where (e=>e.DumpFor ==psbyfilenum.Key).Where (e=>e.Response!=null).Select(e=>e.Response).Sum ()+0.0)/psbyfilenum.Where (e=>e.DumpFor ==psbyfilenum.Key).Select(e=>e.Query).Sum (),
	mDelay=psbyfilenum.Where (e=>e.DumpFor ==psbyfilenum.Key).Where (e=>e.Response!=null).Select(e =>e.Response_delayFirst).Average ()
	};
	ps.Dump ();